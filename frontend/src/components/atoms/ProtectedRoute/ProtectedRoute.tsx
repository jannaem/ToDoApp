...
const ProtectedRoute = ({
  ...
  useEffect(() => {
    getPageToRender(acceptedRoles, auth, redirect).then((page) =>
...
const getPageToRender = async (
  acceptedRoles: Roles[],
  { hasRole, isProcessingAuthentication }: AuthenticationContextProps,
  redirect: string
) => {
  if (isProcessingAuthentication) {
    return <></>;
  }
  return hasRole(acceptedRoles) ? <Outlet /> : <Navigate to={redirect} />;
};
